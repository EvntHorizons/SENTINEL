...
from sentinel.core.time_utils import (
    Time, ensure_time, to_jd, to_mjd, utc_to_tt, utc_to_tdb, utc_to_ut1, gmst, elapsed_sec, epoch_range
)
...
def test_utc_tdb_roundtrip():
    ...
    t_back = Time(t_tdb.jd - 69.184/86400.0) # approx
    assert abs((t_back - t_utc).sec) < 1.0

def test_tt_delta():
    t = ensure_time("2026-03-05T12:00:00")
    tt  = utc_to_tt(t)
    diff = (tt.jd - t.jd) * 86400.0
    assert abs(diff - 69.184) < 0.5
...
def test_elapsed_sec():
    ...
    assert abs(elapsed_sec(t1, t2) - 3600.0) < 1e-4
...
def test_epoch_range_endpoints():
    ...
    assert len(times) == 7
    assert abs((times[-1].jd - ensure_time(stop).jd)*86400.0) < 1e-6
